import React from "react";
import { ReservationsItem } from "components/atoms/ReservationsItem/ReservationsItem";
import Door from "../../../assets/door.svg";
import Book from "../../../assets/book.svg";
import Phone from "../../../assets/phone.svg";
import "./reservations-section.scss";
import { useState, useEffect } from "react";

export const ReservationsSection = () => {
  const APIurl = "https://api.github.com/users/deekshasharma";

  const [userData, setUserData] = useState({});

  useEffect(() => {
    getAPIurl();
  }, []);

  const getAPIurl = async () => {
    const response = await fetch(APIurl);
    const jsonData = await response.json();
    setUserData(jsonData);
  };
  const superduper = {
    userData: [
      {
        userName: "Aubrey Plaza",
        userImage:
          "https://images.unsplash.com/photo-1494790108377-be9c29b29330?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=934&q=80",
        birthdayDate: "1990-05-18",
        location: "A. Juozapavičiaus pr. 11 D, Kaunas",
        reservations: {
          books: [
            {
              id: "1jd85opw82",
            },
            {
              id: "2l36DmN5r10C",
            },
          ],
          devices: [
            {
              id: "1563167577",
            },
            {
              id: "1563168964",
            },
            {
              id: "1563175150",
            },
          ],
          rooms: [
            {
              id: "497ee32e-bd90-4515-94dc-4bc408345845",
            },
          ],
        },
        notifications: [
          {
            userName: "Sara Lars",
            actionType: "Like",
            postId: "10972har27",
          },
        ],
        liked: {
          restaurants: [
            {
              id: "0px1y6iixy",
            },
            {
              id: "hvoirynmd4",
            },
            {
              id: "7et2tk3ih1",
            },
            {
              id: "tk3ih1o0qt",
            },
          ],
          books: [
            {
              id: "1jd85opw82",
            },
            {
              id: "9781484231791",
            },
            {
              id: "9781484234792",
            },
            {
              id: "9781491926789",
            },
            {
              id: "9781680502633",
            },
            {
              id: "9781449318833",
            },
            {
              id: "n1NjDwAAQBAJ",
            },
            {
              id: "4mmarVlnc8oC",
            },
            {
              id: "hMtCDwAAQBAJ",
            },
            {
              id: "UdDkDAAAQBAJ",
            },
            {
              id: "jbhtLNrheUYC",
            },
            {
              id: "dgtfDwAAQBAJ",
            },
            {
              id: "sYE3XOtxxRUC",
            },
            {
              id: "I7ICc40cO9UC",
            },
            {
              id: "viEgDgAAQBAJ",
            },
            {
              id: "7OE5mmdrNYMC",
            },
            {
              id: "JccTct3j3DUC",
            },
            {
              id: "LzwiBQAAQBAJ",
            },
            {
              id: "myivDQAAQBAJ",
            },
            {
              id: "viEgDgAAQBAJ",
            },
            {
              id: "84vD69aw4jkC",
            },
            {
              id: "f65IMQAACAAJ",
            },
            {
              id: "G_kBDAAAQBAJ",
            },
            {
              id: "y3buUQb4hWkC",
            },
            {
              id: "ugbmAgAAQBAJ",
            },
            {
              id: "Ecp_b50eIboC",
            },
            {
              id: "_Zl8AQAAQBAJ",
            },
            {
              id: "2l36DmN5r10C",
            },
          ],
          devices: [
            {
              id: "1563175150",
            },
            {
              id: "1563189600",
            },
            {
              id: "1563172087",
            },
            {
              id: "1563169526",
            },
            {
              id: "1563171668",
            },
            {
              id: "1563182132",
            },
            {
              id: "1563167118",
            },
            {
              id: "1563167188",
            },
            {
              id: "1563168018",
            },
            {
              id: "1563176859",
            },
            {
              id: "1563175935",
            },
            {
              id: "1563176345",
            },
            {
              id: "1563168964",
            },
            {
              id: "1563169643",
            },
            {
              id: "1563169834",
            },
            {
              id: "1563502885",
            },
            {
              id: "1563521285",
            },
          ],
          stories: [
            {
              id: "10187tep27",
            },
          ],
          rooms: [],
        },
        checkIn: {},
        rated: {
          books: [],
        },
        id: "470f8bfe-9105-4d86-bf82-f456cfc803fe",
      },
    ],
  };

  const reservations = [
    {
      id: 1,
      reservationName: "Meeting rooms",
      reservationNumber: 2,
      reservationImage: Door,
    },
    {
      id: 2,
      reservationName: "Books",
      reservationNumber: 2,
      reservationImage: Book,
    },
    {
      id: 3,
      reservationName: "Devices",
      reservationNumber: 2,
      reservationImage: Phone,
    },
  ];
  // userData[0].reservations.books.map(object=> {object.id}))
  return (
    <div className="reservations-section">
      {reservations.map((obj) => {
        return (
          <ReservationsItem
            reservationName={superduper.userData[0].userName}
            // reservationName={userData.name}
            reservationNumber={userData.followers}
            reservationImage={obj.reservationImage}
            key={obj.id}
          />
        );
      })}
    </div>
  );
};
